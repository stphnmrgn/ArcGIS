...
Purpose:     	Process for extracting information from features in a geodatabase
...
# Return base name of pathway (name of folder), strip extension off workspace name, 
# then store folder name as string
fullname = os.path.basename(workspace)
name = fullname.strip(".gdb" )
path_csv = os.path.dirname(workspace) + os.sep + name + "_datadictionary.csv"
...
def describe_data(folder):
    """
    Given a path to a geodatabase, iterate over all features and feature datasets
    and return information about each feature

    Parameters
    ----------
    folder: string. A folder pathway to a file geodatabase

    Returns
    -------
    result: sequence of properties describing each feature within a file geoddatabase
    """
