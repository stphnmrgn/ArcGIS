...
# Strip off GDB file extension and store as string variable for the 'path_csv'
name = fullname.strip(".gdb")
print name

# Store folder name as string variable for the 'path_csv'
path_csv = os.path.dirname(workspace) + os.sep + name + ".csv"
...
with open(path_csv, 'ab') as f:
    ...
    header = ("Feature_Dataset", "Feature_Class", "Shape_Type", "Feature_Description",
              "Spatial_Reference", "Editor", "Workspace", "Populated")
    writer.writerow(header)
    # Loop through feature classes
    for fc in fcList:
        # Create describe objects of features
        desc = arcpy.Describe(fc)
        sr = desc.spatialReference.name  # spatial reference
        st = desc.shapeType  # geometry shape
        # Return dataType object so that Editor Tracking objects can be returned
        dt = desc.dataType
        ...
        writer.writerow(['"Not in Feature Dataset"', fc, st,
                         '"Feature Description"', sr, who, gdb, pop])
    ...
    for dataset in datasetList:
        ...
        for fc in setList:
            ...
            gdb = desc.name
            writer.writerow(
                [dataset, fc, st, '"Feature Description"', sr, who, gdb, pop])
